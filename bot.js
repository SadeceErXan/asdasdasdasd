...
client.on("userUpdate", async (oldUser, newUser) => {
  ...
  await client.guilds.cache.get(sunucu).members.cache.get(newUser.id).send(`**Selam Dostum** ${newUser.username}, **Sunucumuzda _${tag}_ Tagımızı Aldığın İçin** ${client.guilds.cache.get(sunucu).roles.cache.get(rol).name} **Rolünü Sana Verdim! Seviliyosun Dostum ;)**`)
  }
  if (!newUser.username.includes(tag) && client.guilds.cache.get(sunucu).members.cache.get(newUser.id).roles.cache.has(rol)) {
  ...
  await client.guilds.cache.get(sunucu).members.cache.get(newUser.id).send(`**Selam Dostum** ${newUser.username}, **Sunucumuzda _${tag}_ Tagımızı Çıkardığın İçin** ${client.guilds.cache.get(sunucu).roles.cache.get(rol).name} **Rolünü Senden Aldım! Umarım Tekrar Tagımızı Alıp Bize Destek Olursun :)**`)
  }
} catch (e) {
